...
const Pies: FunctionComponent<PiesProps> = ({
  ...
  return (
    <div className="relative w-full h-100 flex flex-col bg-black/60 rounded-lg gap-2 items-center justify-center p-3">
      ...
      <div className="relative flex flex-row gap-4 flex-wrap items-center justify-center overflow-y-scroll h-fit w-full">
        {Array.from({ length: 12 }).map((_, index: number) => {
          const percentage = Number(
            topAccountsFollowed[index]?.percentage || 0
          );
          const numBars =
            Math.ceil(percentage / 5) > 15 ? 15 : Math.ceil(percentage / 5);
          return (
            <div
              className="relative w-20 h-fit flex flex-col font-earl text-white items-center justify-center"
              key={index}
            >
              <Image
